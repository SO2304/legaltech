  ...
  try {
    ...
    if (notifications.avocatNotified) {
      await prisma.dossier.update({
        where: { id: dossierId },
        data: {
          statut: 'ANALYSE_TERMINEE', // Remplacé NOTIFIE (inexistant) par ANALYSE_TERMINEE
    ...
    // Marquer en erreur - remettre à BROUILLON pour permettre nouvelle tentative
    await prisma.dossier.update({
      where: { id: dossierId },
      data: { statut: 'BROUILLON' }, // Remplacé EN_ATTENTE (inexistant) par BROUILLON
    })
